...
o	Grafana는 데이터를 직접 수집하지 않음 → 외부 데이터 소스를 연결해야 함.
o	예시: Prometheus(메트릭), Loki(로그), Tempo(트레이싱), MySQL/Elasticsearch 등.

2.	Dashboard (대시보드)
o	데이터를 시각화하는 UI 컴포넌트.
o	패널(Panel) 단위로 구성 → 차트, 게이지, 테이블 등으로 표현.
o	팀/서비스별로 맞춤형 대시보드 제작 가능.


3.	Panel (패널)
o	대시보드 안에서 실제 데이터를 표현하는 개별 단위.
o	예: CPU 사용량 라인차트, 메모리 게이지, 로그 테이블.


4.	Query Engine (쿼리 엔진)
o	PromQL, LogQL, SQL 등 데이터 소스별 질의어를 실행.
o	대시보드 패널이 실시간으로 데이터를 가져오도록 지원.


5.	Alerting (알람)
o	특정 조건을 만족하면 Slack, Teams, Email 등으로 알림 발송.
o	Prometheus Alertmanager와 달리, 여러 데이터 소스를 동시에 대상으로 삼을 수 있음.


6.	User & Team Management (사용자/권한 관리)
o	조직 단위로 사용자/권한(Role)을 관리 가능.
o	운영팀, 개발팀, 경영진 등 각자 다른 대시보드 권한 부여.


7.	Plugin System (플러그인 시스템)
o	커뮤니티/기업이 만든 다양한 플러그인 설치 가능.
o	데이터 소스 플러그인, 시각화 플러그인, 앱 플러그인 등.
